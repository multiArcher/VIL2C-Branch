#!/bin/bash
# YMAPPO (Yu et al. MAPPO) on SMAC 5m_vs_6m.
# Mirrors on-policy train_smac_5m_vs_6m.sh: 8 envs x 400 steps, 10M, clip 0.05, 10 epochs.

EXPERIMENT_NAME=ymappo_5m_vs_6m
CONFIG=ymappo
ENV_CONFIG=sc2
MAP_NAME=5m_vs_6m
REPEAT_TIMES=1
...
echo "$(timestamp) | INFO     | bash         | Train YMAPPO. Experiment: $EXPERIMENT_NAME" | tee -a "$std_log_path"
